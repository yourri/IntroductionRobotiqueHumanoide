...
for i=1:7 
    ...
    if mod(i, 2) == 0 
        ...
        if i==1 
            ...
            trajectoire_x = creation_trajectoire_pied_x(p_ankle_r_int(1), p_ankle_r_int(1) + dX, 0, temps_un_pas, Ts);
        end
    ...
    % Single support
     for j=1+((i-1) * length(trajectoire_x)):((i-1) * length(trajectoire_x)) + length(trajectoire_x)
        index = j - (i-1) * length(trajectoire_x);
        if mod(i,2) == 0
            Lfoot.p = p_ankle_l_int + [trajectoire_x(index) 0 trajectoire_z(index)']';
            ...
            Rfoot.p = p_ankle_r_int + [trajectoire_x(index) 0 trajectoire_z(index)']';
            Rfoot.R = RPY2R([0, 0, 0]); 
    ...
    p_body = uLINK(MP_BODY).p;
     % DoubleSupport
     for j=1+((i-1) * length(trajectoire_x)) + length(trajectoire_x):1+((i-1) * length(trajectoire_x)) + length(trajectoire_x) + 10          
        uLINK(MP_BODY).p = p_body + [comx(j)+0.015 comy(j) 0]';
        Rfoot.p = [p_ankle_r_int(1), p_ankle_r_int(2), p_ankle_r_int(3)]';
        Rfoot.R = RPY2R([0, 0, 0]); 
        rerr_normr = InverseKinematics(MP_ANKLE2_R, Rfoot);
